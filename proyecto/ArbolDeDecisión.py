...
class Tree:
    ...
    """
    def __init__(self, rows, labels, questionsused, limite = 6):
        self.labels = labels
        ...
        if self.gain == 0 or self.limite == 0:
            ...
            self.hijoTrue = None
            self.hijoFalse = None
            self.prediction = prediction(classCounts(self.rows))
        else:     
...
#def classify(tree, rows, labels):
#    """Uses the decision tree to test new data.
#    """
#    if tree.gain == 0 or tree.limite == 0:
#        if tree.prediction >=0.5:
#            return tree.prediction, "Successfull"
#        else:
#            return tree.prediction, "Unsuccessfull"
#        
#    trueRows, falseRows = partition(rows, tree.question)
#    return classify(tree, trueRows, labels)
#    return classify(tree, falseRows, labels)
                
class Classify:
    def __init__(self, row, tree):
        self.row = row
        self.tree = tree
        self.question = self.tree.question
        if self.tree.gain == 0 or self.tree.limite == 0:
            print("The student will have a", self.tree.prediction["1"], "chance of being successfull")

        else:
            if row[self.tree.question.column] == self.tree.question.value:
                Classify (row, self.tree.hijoTrue)
            else: 
                Classify (row, self.tree.hijoFalse)
            
            
    def __str__ (self):
        print(str(self.tree.prediction))
        

"LECTURA DE ARCHIVO"
archivoTrain = os.path.expanduser('~/Desktop/Datos proyecto/datos5.csv')  
archivoTest = os.path.expanduser('~/Desktop/Datos proyecto/datos0.csv')
#cProfile.run()
