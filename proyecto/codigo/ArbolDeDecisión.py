...
def decidePartition (rows, labels):
...
class Tree:
    ...
    def __init__(self, rows, labels):
        self.labels = labels
        self.gain, self.question = decidePartition(rows, labels)
        if self.gain == 0:
            self.rows = rows
            self.hijoTrue = None
            self.hijoFalse = None
            ...
            self.hijoTrue = Tree(true_rows)
            self.hijoFalse = Tree(false_rows)
            self.miString = Tree.generateString(self)
    ...
    def generateString(self):
        ...
        if (self.hijoTrue != None):
            parameterHijo, comparisonHijo, valuesHijo = self.hijoTrue.question.toString()
            parameterHijoString = self.labels[int(parameter)]
            string = parameterString + comparison + values + " -> " + parameterHijoString + comparisonHijo + valuesHijo
            print(string)
            return string
        if(self.hijoFalse != None):
            parameterHijo, comparisonHijo, valuesHijo = self.hijoFalse.question.toString()
            parameterHijoString = self.labels[int(parameter)]
            string = parameterString + comparison + values + " -> " + parameterHijoString + comparisonHijo + valuesHijo
            print(string)
            return string
